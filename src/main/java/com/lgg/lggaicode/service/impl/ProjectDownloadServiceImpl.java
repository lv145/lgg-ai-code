...
public class ProjectDownloadServiceImpl implements ProjectDownloadService {
    ...
    public void downloadProjectAsZip(String projectPath, String downloadFileName, HttpServletResponse response) {
        ...
        log.info("开始压缩项目目录: {}, 下载文件名: {}", projectPath, downloadFileName);
        //设置Http响应头
        ...
        try {
            //参数一：输出流 参数二：字符集 参数三：是否递归压缩子目录 参数四：文件过滤器 参数五：项目目录
            ZipUtil.zip(response.getOutputStream(), StandardCharsets.UTF_8,true,filter,projectDir);
            log.info("项目目录压缩完成，下载文件名: {}", downloadFileName);
        }catch (Exception e) {
            log.error("压缩项目目录失败", e);
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "压缩项目目录失败");
        }
    }
}
